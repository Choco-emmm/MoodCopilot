                ...
                try {
                    ...
                    if ((!diffAdded.isEmpty() || !diffDeleted.isEmpty())
                        ...
                        for (java.util.Map<String, String> item : diffAdded) {
                            md.append("<div style=\"color: var(--color-success)\">+ ").append(item.get("head")).append(" —[").append(item.get("relation")).append("]→ ").append(item.get("tail")).append("</div>");
                        }
                        for (java.util.Map<String, String> item : diffDeleted) {
                            md.append("<div style=\"color: var(--color-error); text-decoration: line-through;\">- ").append(item.get("head")).append(" —[").append(item.get("relation")).append("]→ ").append(item.get("tail")).append("</div>");
                        }
