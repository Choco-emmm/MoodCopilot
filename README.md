...
<p align="center">
  <strong>🫂 懂你的开源通用个人 AI</strong><br />
  <em>“记录生活，理解自己，也把每一次对话变成更好的陪伴。”</em>
</p>
...
> **👋 寻找种子用户与贡献者！**
> MoodCopilot 已经正式上线并稳定运行中！**（我本人每天都在上面写日记，所以这是一个绝对不会删档的长期服务，请放心把你的情绪交给MoodCopilot妥善保管。）** 无论你是想寻找一个专属的赛博树洞，还是想探索前沿的 AI 应用开发（RAG、双模型、多模态），都非常欢迎你的加入！去创造专属于你的情绪自留地吧。

**MoodCopilot** 是一款基于大语言模型构建的通用个人 AI 应用，提供日记、聊天、长期记忆、情绪陪伴和社区能力。它可以记录文本、音乐和图片，也能处理编程、学习、写作、翻译和规划等日常任务，帮助用户理解自己的情绪、回顾个人经历并持续修正长期画像。MoodCopilot 提供情绪支持，不做临床诊断，也不把模型推断当作未经确认的用户事实。
...
*   **双模型与深度思考**：底层全面接入 **deepseek-v4-flash** 与 **deepseek-v4-pro**。聊天和日记分析前由用户手动选择普通模型或 **pro 模型的深度思考模式（Reasoning）**；系统不会根据问题内容自动判断模型，也不会在额度不足时静默切换模型。
*   **重要事件回访**：从日记中提取仍在持续的个人事件，聚合相关日记并提供私密的回访入口；支持开始/结束日期、可选时间点或时间范围、手动创建、编辑和日记关联。事件只保留“待跟进”和“已跟进”两个用户状态，状态变更可撤销。
*   **时光画卷**：按时间段沉淀个人成长章节，展示阶段主题、主导情绪和成长反思，不把章节改造成公开社交内容。
*   **可纠正的长期记忆**：明确表达形成正式记忆，模型推断先进入候选区；记忆带有来源、证据、置信度、有效期和版本历史，用户可以确认、修改或删除，所有正式写入统一经过 Memory Orchestrator。
...
| **异步任务** | RabbitMQ 4 durable queues + MySQL task/outbox 状态表；Redis 不承担 AI 任务事实状态 |
...
Redis 负责缓存、限流、聊天历史和 RAG 向量检索；RabbitMQ 只负责投递异步 AI 任务。任务状态、幂等键、租约、重试和失败原因以 MySQL 任务表为准，消息只携带任务索引和用户隔离所需的标识，不携带日记正文或聊天内容。聊天和日记分析中的 Pro/Flash 始终由用户手动选择：`useReasoning=false` 使用 Flash，`useReasoning=true` 使用 Pro，系统不会根据问题复杂度自动路由或静默降级。RAG 结果只作为参考资料，不具备系统指令权限；候选记忆的证据必须能定位到用户原文，无法匹配时不会保存整篇日记作为依据。
...
- 或者手动配置：Java 21+, Node.js 20+, MySQL 8.0, Redis Stack, RabbitMQ 4.x
...
│       ├── ai/                  # 🤖 AI 编排：用户选择的双模型、VLM 视觉、长期记忆与任务处理
│       ├── event/               # 🧵 私密重要事件与时光画卷章节
