...
**MoodCopilot** 是一款基于大语言模型构建的情绪日记与社交应用，也是一位面向长期私人陪伴的 AI 伙伴。它可以记录纯文本、音乐和图片，帮助用户理解自己的情绪、回顾个人经历并持续修正长期画像。MoodCopilot 提供情绪支持，不做临床诊断，也不把模型推断当作未经确认的用户事实。
...
Redis 负责缓存、限流、聊天历史和 RAG 向量检索；RabbitMQ 只负责投递异步 AI 任务。任务状态、幂等键、租约、重试和失败原因以 MySQL 任务表为准，消息只携带任务索引和用户隔离所需的标识，不携带日记正文或聊天内容。聊天和日记分析中的 Pro/Flash 始终由用户手动选择：`useReasoning=false` 使用 Flash，`useReasoning=true` 使用 Pro，系统不会根据问题复杂度自动路由或静默降级。
