...
const navItems = computed(() => {
  ...
  if (auth.isAdmin) {
    items.push(
      { label: '审核', shortLabel: '审核', icon: adminIcon, path: '/admin/reports', cls: 'nav-link-admin desktop-only' },
      { label: '用户', shortLabel: '用户', icon: usersIcon, path: '/admin/users', cls: 'nav-link-admin desktop-only' },
    )
  }
  return items
})
