  ...
  <main class="app-shell life-page">
    ...
    <section class="event-list" aria-live="polite">
      ...
      <div v-else-if="events.length === 0" class="state empty">
        <p>暂时没有重要事件。</p>
        <p class="state-hint">记下一件想回来关注的事，之后可以让 MoodCopilot 适时问问你。</p>
        <button class="add-button" type="button" @click="openCreate">＋ 添加事件</button>
      </div>
...
.state { padding: 42px 0; color: var(--color-text-muted); text-align: center; }.state p { margin: 0 0 8px; }.state-hint { font-size: 13px; }.state.error, .editor-error { color: var(--color-error); }
